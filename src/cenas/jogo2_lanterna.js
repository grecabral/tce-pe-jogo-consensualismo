// Jogo 2 — Lanterna: revelar instrumentos em cenário escuro.
...
export function montar(app, ctx) {
  return {
    onEnter() {
      ...
      app.innerHTML = `
        <section class="cena cena-jogo2" id="cena-jogo2">
          ...
          <div class="lanterna-onboarding" id="lanterna-onboarding">${lanternaCfg.dicaOnboarding || t.instrucao}</div>
      ...
      root = app.querySelector('#cena-jogo2');
      ...
      // Spawn dos itens — posições aleatórias em grid com folga das bordas.
      const itens = [
      ...
      function atualizarMascara() {
        ...
        // Revela itens dentro do círculo.
        itensEls.forEach((el) => {
          ...
          el.classList.toggle('revelado', dist <= RAIO);
        });
      ...
      function onTap(ev) {
        ...
        if (data.tipo === 'instrumento') {
          ...
          if (coletados >= COLETAR_ALVO) {
            setTimeout(() => irPara('JOGO3_COLETOR'), 1800);
          }
        } else {
...
function gerarPosicoes(n, palco) {
  // Distribui em grid simples com jitter, evitando bordas.
  ...
  for (let i = 0; i < n; i++) {
    ...
    ps.push({
      x: margemX + (cols > 1 ? c * passoX : 50 - margemX) + jitterX,
      y: margemY + (rows > 1 ? r * passoY : 50 - margemY) + jitterY,
    });
  }
